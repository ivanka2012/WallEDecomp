...
public:
	...
	void	SetSize(S32 NewSize,Bool ResizeOnlyIfGreater = FALSE)
	{
		...
		if( (U32) NewSize > Size )
		{
			if( ((U32)(NewSize)-Size>ReservedSize) || !ResizeOnlyIfGreater )
			{
				Realloc(NewSize);
				ReservedSize = 0;
			}
			else if( ((U32)(NewSize)-Size<=ReservedSize))
			{
...
private:
	void	Realloc(int	NewNbElement)
	{
		...
		{
			if(ArrayPtr)
				ArrayPtr=(T *)Realloc_Z(ArrayPtr, NewNbElement*sizeof(T)); //Align maybe?
			else
				ArrayPtr=(T *)Alloc_Z(NewNbElement*sizeof(T));
